...
class CloseorderService {
  Future<APIResponse<String>> closeOrdObj(
    ...
    List<String> fieldList = [
      ...
      '<soap:Body>',
      '<GetCloseOrderDetails xmlns="http://tempuri.org/">',
      '<Obj>$obj</Obj>',
      '</GetCloseOrderDetails>',
      '</soap:Body>',
      '</soap:Envelope>'
    ];
    ...
    return await http
        .post(Uri.parse('${Constants.baseUrlTerminal}?op=GetCloseOrderDetails'),
            headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['ReturnCode'] == '400') {
          if (data['ClosedOrder'] != []) {
            List<dynamic> accountDetailsList = jsonDecode(data['ClosedOrder']);
            provider.addCloseOrder(accountDetailsList.map((item) => OpenOrderElement.fromJson(item)).toList());
            _addToControllerIfNotClosed(accountDetailsList, NotifySocketUpdate.controllerClose);
          }
        }
